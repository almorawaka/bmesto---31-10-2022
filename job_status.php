...
<?php
 ...
 if(isset($_POST['submit']))
 {
     ...
     $resons = mysqli_real_escape_string($con,$_POST['resons']);
     $submittedby = $_SESSION['username'];  
     echo $submittedby	 ;
 
 // Creating an insert query using SQL syntax and storing it in a variable.
        $sql_insert =  "INSERT INTO `status_tbl`(`job_id`, `status`, `resons`, `submittedby`) VALUES ('$j_id','$status','$resons','$submittedby')";
    ...
    <div class="card-header">
      ...
      <!-- from start -->
           <form method="POST" class="row g-3">
            ...
            <div class="col-md-6">
              <label class="form-label" >Resons</label>
              <label for="inputAddress" class="form-label"></label>
              <input type="text" class="form-control" name="resons" placeholder=" Reson" required>
            </div>
